...
export const Net = {
  ...
  connect(room, name, role, seed) {
    ...
    this.ws = ws;
    ws.onopen = () => ws.send(JSON.stringify({ t: "join", room, name, role, seed }));
  ...
  _recv(m) {
    switch (m.t) {
      case "welcome":
        this.on = true; this.id = m.id; this.isHost = !!m.host; this.room = m.room; this.seed = m.seed;
        this.peers.clear();
